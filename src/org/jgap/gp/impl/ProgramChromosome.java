  ...
  public synchronized Object clone() {
    try {
      ...
      for (int i = 0; i < size; i++) {
        ...
        // Try deep clone_ICloneable of genes.
        // ------------------------
        if (ICloneable.class.isAssignableFrom(m_genes[i].getClass())) {
          genes[i] = (CommandGene) ( (ICloneable) m_genes[i]).clone_ICloneable();
        }
        else {
          // No deep clone_ICloneable possible.
          // -----------------------
          genes[i] = m_genes[i];
        }
      }
  ...
  protected CommandGene[] growOrFullNode(int a_num, int a_depth,
    ...
    if (a_rootNode == null || a_validateNode) {
      ...
      do {
        ...
        // Optionally use a mutant/clone_ICloneable of the originally selected command
    ...
    if (uncloned && !conf.isNoCommandGeneCloning()
        ...
        /**@todo we could optionally use the clone_ICloneable handler*/
      a_rootNode = (CommandGene) ( (ICloneable) a_rootNode).clone_ICloneable();
