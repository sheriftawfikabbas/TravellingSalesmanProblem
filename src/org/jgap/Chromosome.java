...
public class Chromosome
   ...
   * @since 1.0
   */
  public synchronized Object clone_ICloneable() {
    ...
    try {
      if (copy == null) {
        ...
        if (size > 0) {
          ...
          // return it. Also clone_ICloneable the IApplicationData object later on.
          // ---------------------------------------------------------------
          if (getClass() == Chromosome.class) {
            ...
            try {
              ...
              // --------------------------------------------------------------
              copy = (IChromosome) getConfiguration().getSampleChromosome().clone_ICloneable();
              copy.setGenes(copyOfGenes);
            }
          }
        }
    ...
    // Also clone_ICloneable the IApplicationData object.
    // ---------------------------------------
    try {
  ...
  /**
   * Clones an object by using clone_ICloneable handlers. If no deep cloning possible, then
   * return the reference.
   *
   * @param a_object the object to clone_ICloneable
  ...
  public static Object cloneObject(Configuration a_config, Object a_object,
    ...
    // Try to clone_ICloneable via a registered clone_ICloneable handler.
    // --------------------------------------------
  ...
  public static IChromosome randomInitialChromosome(
    ...
    for (int i = 0; i < newGenes.length; i++) {
      ...
      // If application data is set, try to clone_ICloneable it as well.
      // ----------------------------------------------------
