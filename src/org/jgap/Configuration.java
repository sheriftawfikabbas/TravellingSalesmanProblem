...
//  }
  ...
  /**
   * @return deep clone_ICloneable of this instance
   *
   * @author Klaus Meffert
   * @since 3.2
   */
   ...
   * ID and the name.
   *
   * @param a_id new ID for clone_ICloneable
   * @param a_name new name for clone_ICloneable
   * @return deep clone_ICloneable of this instance
   *
   * @author Klaus Meffert
   * @since 3.2
  ...
  public Configuration newInstance(String a_id, String a_name) {
    try {
      ...
      if (m_factory instanceof ICloneable) {
        result.m_factory = (IJGAPFactory)((ICloneable)m_factory).clone_ICloneable();
      }
      else {
        // We must fallback to a standardized solution.
        // --------------------------------------------
        m_factory = new JGAPFactory(false);
        result.m_factory = (IJGAPFactory)((JGAPFactory)m_factory).clone_ICloneable();
      }
      if (m_breeder != null) {
...
//    result.m_chromosomePool = m_chromosomePool.clone_ICloneable();
//    result.m_conHandler = m_conHandler.clone_ICloneable();
...
//      result.m_popConstantSelector = (INaturalSelector)doClone(m_popConstantSelector);
      ...
      if (m_sampleChromosome != null) {
        result.m_sampleChromosome = (IChromosome) m_sampleChromosome.clone_ICloneable();
      }
...
//      result.m_propertyBag = (Map)doClone(m_propertyBag);
  ...
  /**
   * Helper called from clone_ICloneable.
   *
   * @param a_objToClone the object to clone_ICloneable
