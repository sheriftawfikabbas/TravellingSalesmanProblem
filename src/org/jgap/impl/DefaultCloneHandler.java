...
/**
 * Default clone_ICloneable handler supporting IApplicationData as well as implementations
 * of Cloneable (for the latter: in case the clone_ICloneable-method is accessible via
 * reflection).
 *
...
public class DefaultCloneHandler
   ...
   * java.lang.Cloneable (for which the clone_ICloneable-method is accessible via
  ...
  /**
   * Performs the clone_ICloneable operation.
   *
  ...
  public Object perform(final Object a_objToClone, final Class a_class,
    ...
    if (ICloneable.class.isAssignableFrom(clazz)) {
      try {
        return ( (ICloneable) a_objToClone).clone_ICloneable();
      }
    ...
    // Support Cloneable interface by looking for clone_ICloneable() method
    // via introspection.
    // ---------------------------------------------------------
    try {
