...
public abstract class Salesman
    ...
    public Configuration createConfiguration(final Object a_initial_data)
        ...
        config.addGeneticOperator(new SwappingMutationOperator(config, 10));
        config.addGeneticOperator(new SHERIF_GreedyCrossover(config,this));
        return config;
    }
    ...
    public IChromosome findOptimalPath(final Object a_initial_data)
        ...
        // ---------------------------------------------------------------
        System.out.println("Evolution starts-------------------------------------");
        Evolution:
        for (int i = 0; i < getMaxEvolution(); i++) {
            System.out.println(" ####### Evolution Cycle " + i + " ######## ");
            population.evolve();
            ...
            System.out.println(" Distance = " + -(best.getFitnessValue() - Integer.MAX_VALUE / 2));
    ...
    /*
     * Sherif
     * Added the JFreeChart, Graphics object to plot routes
     */
    /**
     * Executes the genetic algorithm to determine the
     * optimal path between the cities.
     ...
     * @since 2.0
     */
    public IChromosome findOptimalPath(final Object a_initial_data, JFreeChart _chart,TSPPane pane)
            throws Exception {
        ...
        for (int i = 0; i < getMaxEvolution(); i++) {
            ...
            series.add(m_config.getGenerationNr(), Integer.MAX_VALUE / 2-best.getFitnessValue());
